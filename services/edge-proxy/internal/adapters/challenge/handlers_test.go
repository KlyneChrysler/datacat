...
func testHandlers() (*Handlers, *app.Challenger) {
	challenger := app.NewChallenger("test-secret", 4)
...
func solve(t *testing.T, challenger *app.Challenger, token string) string {
	t.Helper()
	now := time.Now()
...
func hasValidClearance(cookies []*http.Cookie, challenger *app.Challenger) bool {
	for _, cookie := range cookies {
		if cookie.Name == app.ClearanceCookie &&
