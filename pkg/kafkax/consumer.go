...
// Handler processes one record and must be idempotent.
type Handler func(ctx context.Context, key, value []byte) error

// Consumer reads one topic with at least once delivery.
...
func NewConsumer(brokers, group, topic string, log *slog.Logger) (*Consumer, error) {
	client, err := kgo.NewClient(kgo.SeedBrokers(splitBrokers(brokers)...), kgo.ConsumerGroup(group), kgo.ConsumeTopics(topic), kgo.DisableAutoCommit())
	if err != nil {
...
// Consume handles records until ctx ends, skipping poison records.
...
func (c *Consumer) handleRecord(ctx context.Context, r *kgo.Record, handle Handler) {
	if err := handle(ctx, r.Key, r.Value); err != nil {
		c.log.ErrorContext(ctx, "record handling failed, skipping", "err", err, "topic", r.Topic, "partition", r.Partition, "offset", r.Offset)
	}
}
...
func (c *Consumer) logFetchErrors(ctx context.Context, fetches kgo.Fetches) {
	for _, fe := range fetches.Errors() {
		c.log.ErrorContext(ctx, "fetch error", "err", fe.Err, "topic", fe.Topic, "partition", fe.Partition)
	}
}
